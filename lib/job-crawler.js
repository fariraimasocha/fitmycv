...
// Relative, not "@/lib/groq": `npm run check:jobs` runs this file under plain node.
import { chat, MODEL_FAST } from "./groq.js";
...
export async function enrichJobs(jobs) {
  if (!process.env.GROQ_API_KEY || jobs.length === 0) return [];
  ...
  for (let i = 0; i < batches.length; i += ENRICH_CONCURRENCY) {
    ...
    const settled = await Promise.allSettled(wave.map((b) => enrichBatch(b)));
    settled.forEach((r, n) => {
