...
5. Generate cover letter. OpenAI creates a cover letter from the tailored CV plus job context
...
1. Upload a PDF CV, then verify it parses and saves to ReferenceCV model
2. Visit My CV page, then verify stored CV displays correctly
3. Re-upload CV, then verify it updates (not duplicates)
4. Paste a LinkedIn job URL, then verify Exa.ai scrapes requirements correctly
5. Generate tailored CV, then verify content is adjusted for the job
6. Generate cover letter, then verify it references the job and user's experience
7. Download PDF, then verify CV and cover letter render as proper PDFs

## Writing Copy

Copy means any text a user reads: page headings, body text, button labels, empty
states, error messages, toasts, tooltips, alt text, and metadata.

- **Before writing or editing copy, invoke the `ux-writing` skill.** It lives in
  `.agents/skills/ux-writing` and covers buttons, errors, empty states, forms,
  and tone. Do not write copy from instinct when the skill is right there.
- **Never use dashes as punctuation in copy.** No em dash, no en dash, no double
  hyphen standing in for one. Use a full stop, a comma, a colon, or split the
  sentence. Hyphens inside a compound word ("full-stack", "opt-in") are fine.
- **Every claim must be verifiable from the data.** Do not write "no expired
  listings" unless something actually filters expired listings. If a claim
  cannot be backed, cut it.
- **Say the thing plainly.** Short sentences, ordinary words, active voice, and
  second person. No marketing puffery.

This applies to code comments and commit messages too, minus the skill step.
