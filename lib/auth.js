...
export const { handlers, auth, signIn, signOut } = NextAuth({
  ...
  // 308 redirect. Without this the first navigation to /dashboard looks
  // unauthenticated and can flash the error boundary until the next reload
  // picks up the host-only cookie. Only in production – on localhost a Domain
  // attribute would prevent the cookie from being set at all.
  ...(process.env.NODE_ENV === "production"
    ? {
        cookies: {
          sessionToken: { options: { domain: ".fitmycv.link" } },
          callbackUrl: { options: { domain: ".fitmycv.link" } },
          csrfToken: { options: { domain: ".fitmycv.link" } },
        },
      }
    : {}),
