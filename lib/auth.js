...
export const { handlers, auth, signIn, signOut } = NextAuth({
  ...authConfig,
  // Share the session cookie between apex and www so a magic-link that was
  // created on fitmycv.link still authenticates on www.fitmycv.link after the
  // 308 redirect. Do not put Domain on the CSRF cookie: Auth.js names it
  // __Host-authjs.csrf-token on HTTPS, and browsers reject __Host- cookies
  // that have a Domain attribute. That left sign-out POSTs without a CSRF
  // cookie, Auth.js sent the user to /auth?error=MissingCSRF, and the
  // authorized callback bounced them back to /dashboard still signed in.
  // Only in production. On localhost a Domain attribute would prevent the
