...
// Weighting ported from Reactive Resume's jd/match.ts: terms under a
// requirements heading count for more than the company blurb, and names on the
// curated skill list count for more than generic words.
const REQUIREMENTS_HEADING =
  /\b(?:requirement|qualification|must[- ]have|nice[- ]to[- ]have|what you(?:'| a|)ll (?:need|bring)|about you|we(?:'| a|)re looking for|your (?:profile|background)|you (?:have|bring)|minimum|preferred)/i;
const REQUIREMENTS_BOOST = 1.5;
const KNOWN_SKILL_BOOST = 2;
const SKILL_FORMS = new Set(SKILL_ALIASES.flatMap(([, aliases]) => aliases));

// A specialist repeats their own field, so calling a term stuffed takes all
// three: many uses, far more than the posting, and a real share of the CV.
...
function extractTerms(text, limit) {
  ...
  const bump = (map, term, inRequirements) => {
    const entry = map.get(term) || { count: 0, inRequirements: false };
    entry.count += 1;
    entry.inRequirements ||= inRequirements;
    map.set(term, entry);
  };

  let inRequirements = false;
  for (const line of text.split(/\n+/)) {
    // A heading flips this line and every line after it into the requirements.
    if (REQUIREMENTS_HEADING.test(line)) inRequirements = true;
    const tokens = tokenize(line);
    tokens.forEach((token, i) => {
      if (isContent(token)) bump(unigrams, token, inRequirements);
      const next = tokens[i + 1];
      if (!isContent(token) || !isContent(next)) return;
      bump(bigrams, `${token} ${next}`, inRequirements);
    });
  }

  const weigh = (term, entry, base) =>
    entry.count *
    base *
    (entry.inRequirements ? REQUIREMENTS_BOOST : 1) *
    (SKILL_FORMS.has(term) ? KNOWN_SKILL_BOOST : 1);
  ...
  const scored = [
    ...[...bigrams.entries()]
      ...
      .map(([term, entry]) => ({
        term,
        count: entry.count,
        score: weigh(term, entry, 1.8),
        words: term.split(" "),
      })),
    ...[...unigrams.entries()].map(([term, entry]) => ({
      term,
      count: entry.count,
      score: weigh(term, entry, 1),
      words: [term],
    })),
...
function keywordCoverage(jobText, cvText, limit = 24) {
  ...
  const cvTokens = tokenize(cvText).map(stem);
  const cvCounts = new Map();
  cvTokens.forEach((token, i) => {
    cvCounts.set(token, (cvCounts.get(token) || 0) + 1);
    const next = cvTokens[i + 1];
    if (next) cvCounts.set(`${token} ${next}`, (cvCounts.get(`${token} ${next}`) || 0) + 1);
  });
  const scored = terms.map((item) => {
    const words = item.words.map(stem);
    return {
      ...item,
      present: words.every((word) => cvCounts.has(word)),
      cvCount: cvCounts.get(words.join(" ")) || 0,
    };
  });
  ...
  return {
    ...
    stuffed: scored
      .filter(
        (item) =>
          item.cvCount >= STUFFING_MIN_COUNT &&
          item.cvCount >= item.count * STUFFING_MULTIPLIER &&
          item.cvCount / cvTokens.length >= STUFFING_MIN_DENSITY
      )
      .map((item) => item.term),
  };
}
