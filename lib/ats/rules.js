...
export const CATEGORIES = {
  contact: { label: "Contact details", weight: 30 },
  sections: { label: "Sections", weight: 35 },
  dates: { label: "Dates", weight: 35 },
  // Tips live here. They are shown beside the score and never counted in it.
  content: { label: "Writing tips", weight: 0 },
};
...
export function scoreCv(findings) {
  ...
  const categories = Object.entries(CATEGORIES)
    ...
    .map(([key, { label, weight }]) => {
      ...
      return { key, label, weight, score: Math.max(0, 100 - penalty), failed: codes.length };
    });
