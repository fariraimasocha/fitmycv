...
// One-click "Save" link for the email — signed so it works without a session.
function saveLink(userId, jobId) {
  const token = `${userId}:${jobId}`;
  const params = new URLSearchParams({ u: userId, j: jobId, s: sign(token) });
  return `${SITE_URL}/api/saved-jobs/save?${params}`;
}
        ...
        <table role="presentation" width="100%" cellpadding="0" cellspacing="0">
          <tr>
            ...
            <td valign="top" align="right" style="padding-left:12px;">
              ...
              ${userId ? `<div style="margin-top:8px;"><a href="${escapeHtml(saveLink(userId, job.id))}" target="_blank" class="em-ink-soft" style="display:inline-block;font-family:${FONT_DISPLAY};font-weight:700;font-size:12px;color:${BRAND.inkSoft};text-decoration:none;">☆&nbsp;Save</a></div>` : ""}
            </td>
          </tr>
          <tr>
...
export function buildJobDigestEmail({
  ...
  const cards = jobs.map((job, i) => jobCard(job, i, userId)).join("");
          ...
          <tr>
            <td class="em-footer" style="background:${BRAND.paperStrong};border-top:1px solid ${BRAND.line};padding:22px 32px;text-align:center;">
              ...
              <p class="em-ink-soft" style="margin:0 0 8px;font-family:${FONT_BODY};font-size:11px;font-weight:600;color:${BRAND.inkSoft};">
                ...
                &nbsp;&middot;&nbsp;
                <a href="${escapeHtml(dashboardUrl)}/preferences" class="em-ink-soft" style="color:${BRAND.inkSoft};text-decoration:none;">Manage preferences</a>
                &nbsp;&middot;&nbsp;
                <a href="${escapeHtml(dashboardUrl)}/preferences" class="em-ink-soft" style="color:${BRAND.inkSoft};text-decoration:none;">Unsubscribe</a>
              </p>
